...
class YOLO:
        ...
        try:
            ...
            for key in data:
                ...
                for idx in range(0, int(data[key]["objects"]["num_obj"])):
                    ...
                    def get_class_index(cls_list, cls_hierarchy, cls_name,flag):
                        if flag:
                            print("clas_name", cls_name)
                            print("index", cls_list.index(cls_name))
                        ...
                        if type(cls_hierarchy) is dict and cls_name in cls_hierarchy:
                            return get_class_index(cls_list, cls_hierarchy, cls_hierarchy[cls_name],flag)
                    ...
                    cls_id = get_class_index(
                        self.cls_list, self.cls_hierarchy, cls_name,flag)
    ...
    def save(self, data, save_path, img_path, img_type, manifest_path,manifest_name):
        ...
        try:
            ...
            if os.path.isdir(manifest_path):
                manifest_abspath = os.path.join(manifest_path, "{}.txt".format(manifest_name))
            else:
